...
export const PollsController = () => {
  ...
  const createVoteClickHandler = (pollId: number) => (choiceId: number) => () => {
    (async () => {
      const requestConfig = {params: {choiceId: choiceId.toString(), id: pollId.toString()}};
      const result = await apiClient.post(`/polls/:id/choices/:choiceId/vote`, requestConfig);
      updatePoll(pollId, result);
    })();
  }

  const createChoiceSubmitHandler = (pollId: number) => (text: string) => {
    (async () => {
      const requestConfig = {
        params: {id: pollId.toString()},
        body: {text},
      };
